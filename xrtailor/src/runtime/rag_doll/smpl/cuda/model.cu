...
__host__ void Model<ModelConfig>::_cuda_free() {
  ...
  if (device.blend_shapes)
    cudaFree(device.blend_shapes);
  if (device.joint_reg_dense)
    cudaFree(device.joint_reg_dense);
  if (device.joint_reg.values)
    cudaFree(device.joint_reg.values);
  if (device.joint_reg.inner)
    cudaFree(device.joint_reg.inner);
  if (device.joint_reg.outer)
    cudaFree(device.joint_reg.outer);
  if (device.weights.values)
    cudaFree(device.weights.values);
  if (device.weights.inner)
    cudaFree(device.weights.inner);
  if (device.weights.outer)
    cudaFree(device.weights.outer);
  if (device.hand_comps_l)
    cudaFree(device.hand_comps_l);
  if (device.hand_comps_r)
    cudaFree(device.hand_comps_r);
  if (device.hand_mean_l)
    cudaFree(device.hand_mean_l);
  if (device.hand_mean_r)
    cudaFree(device.hand_mean_r);
}
