...
#include <xrtailor/runtime/rag_doll/smpl/model_config.hpp>
...
class Model {
 public:
  ...
  void SetDeformations(const Eigen::Ref<const Points>& d);

  // Set model template: verts := t
  void SetTemplate(const Eigen::Ref<const Points>& t);
...
class Body {
 public:
  ...
  //                          of worse accuracy
  void Update(bool force_cpu = false, bool enable_pose_blendshapes = false,
              bool enable_garment_skinning = false, bool enable_slot_skinning = false);
  ...
  __SMPLX_MEMBER_ACCESSOR(
      hand_pca, params.segment<ModelConfig::n_hand_pca() * 2>(3 + 3 * model.n_explicit_joints()));
  __SMPLX_MEMBER_ACCESSOR(hand_pca_l, params.template segment<ModelConfig::n_hand_pca()>(
                                          3 + 3 * model.n_explicit_joints()));
  __SMPLX_MEMBER_ACCESSOR(hand_pca_r, params.template segment<ModelConfig::n_hand_pca()>(
                                          3 + 3 * model.n_explicit_joints() + model.n_hand_pca()));
  // Shape params
  ...
  const Points& host_cloth_verts() const;
  const Points& host_cloth_slots() const;

  const float* device_cloth_verts() const;
  const float* device_cloth_slots() const;

  // Get shaped (but not posed) body vertices, in same order as model.verts;
  // must call update() before this is available
  const Points& verts_shaped() const;
  const Points& cloth_verts_shaped() const;
  ...
  const Eigen::Matrix<Scalar, Eigen::Dynamic, 12, Eigen::RowMajor>& joint_transforms() const;

  // Get homogeneous transforms at each vertex. (n_verts, 12).
  // Each row is a row-major (3, 4) rigid body transform matrix,
  // canonical -> posed space.
  const Eigen::Matrix<Scalar, Eigen::Dynamic, 12, Eigen::RowMajor>& vert_transforms() const;
  ...
  // LBS weights transferred to garment, (#cloth_verts, #joints).
  SparseMatrixColMajor cloth_weights;
  SparseMatrixColMajor cloth_slot_weights;

  void EmbedSkinningWeightsIntoCloth(const SparseMatrixColMajor& body_weights,
                                       SparseMatrixColMajor& cloth_weights,
                                       unsigned int num_garment_vertices,
                                       unsigned int* nearest_body_indices);

  void EmbedBarycentricSkinningWeightsIntoCloth(const SparseMatrixColMajor& body_weights,
                                                  SparseMatrixColMajor& cloth_weights,
                                                  unsigned int num_garment_vertices,
                                                  XRTailor::SkinParam* skin_params);

  void EmbedSkinningWeightsIntoSlots(const SparseMatrixColMajor& body_weights,
                                     unsigned int num_slots, const unsigned int* slot_indices,
                                     unsigned int* nearest_body_indices);

  void EmbedBarycentricSkinningWeightsIntoSlots(const SparseMatrixColMajor& body_weights,
                                                unsigned int num_slots,
                                                const unsigned int* slot_indices,
                                                XRTailor::SkinParam* skin_params);

  void EmbedBlendShapesIntoCloth(unsigned int num_garment_vertices,
                                   const unsigned int* nearest_body_indices);

  void EmbedBarycentricBlendShapesIntoCloth(unsigned int num_garment_vertices,
                                            XRTailor::SkinParam* skin_params);

  void EmbedBlendShapesIntoSlots(unsigned int num_slots, const unsigned int* slot_indices,
                                 const unsigned int* nearest_body_indices);
 ...
 private:
  ...
  mutable Eigen::Matrix<Scalar, Eigen::Dynamic, 12, Eigen::RowMajor> _vert_transforms;
  ...
  // Deformed vertices (only shape applied); not available in case of GPU
  // (only device.verts_shaped)
  mutable Points _garment_verts_shaped;
  mutable Points _garment_slots_shaped;
  // Deformed vertices (shape and pose applied)
  mutable Points _garment_verts;
  mutable Points _garment_slots;
 ...
 public:
  struct {
    ...
    float* initial_cloth_verts = nullptr;
    float* cloth_verts = nullptr;
    float* cloth_verts_shaped = nullptr;

    float* initial_cloth_slots = nullptr;
    float* cloth_slots = nullptr;
    float* cloth_slots_shaped = nullptr;

    float* cloth_blend_shapes = nullptr;
    float* slot_blend_shapes = nullptr;

    internal::GPUSparseMatrix cloth_weights;
    internal::GPUSparseMatrix cloth_slot_weights;
  } device;

  void _cuda_set_cloth_vertices(std::shared_ptr<XRTailor::PhysicsMesh> physics_mesh);
  void _cuda_load_cloth(int _num_verts);
  void _cuda_load_slots(int _num_slots);

 private:
  // True if latest posed vertices constructed by update()
  // have been retrieved to main memory
  mutable bool _verts_retrieved;
  mutable bool _cloth_verts_retrieved;
  mutable bool _cloth_slots_retrieved;
  // True if latest shaped, unposed vertices constructed by update()
  // have been retrieved to main memory
  mutable bool _verts_shaped_retrieved;
  mutable bool _cloth_verts_shaped_retrieved;
  mutable bool _cloth_slots_shaped_retrieved;
  ...
  void _cuda_update(float* h_blendshape_params, float* h_joint_transforms,
                    bool enable_pose_blendshapes = true, bool enable_cloth_skinning = false,
                    bool enable_slot_skinning = false);

  unsigned int num_cloth_verts;
  unsigned int num_slots;

  void _cuda_maybe_retrieve_cloth_verts() const;
  void _cuda_maybe_retrieve_cloth_slots() const;
  void _cuda_maybe_retrieve_cloth_verts_shaped() const;
  void _cuda_maybe_retrieve_cloth_slots_shaped() const;
#endif
};
// SMPL Body
